...
public class SCatSequence extends Notifier {
    ...
    int catAtPos(int id) {
        if (id<0 || id>=cats) return Common.runtimeWarning("SCatSequence on "+v.getName()+": catAtPos("+id+") out of range ("+cats+" cats)");
        return (seqToCat==null)?id:seqToCat[id];
    }

    /** returns the position of the category id */
    int posOfCat(int id) {
        if (id<0 || id>=cats) return Common.runtimeWarning("SCatSequence on "+v.getName()+": posOfCat("+id+") out of range ("+cats+" cats)");
    ...
    public boolean swapCatsAtPositions(int p1, int p2) {
        if (p1<0 || p2<0 || p1>=cats || p2>=cats) return
            Common.runtimeWarning("SCatSequence on "+v.getName()+": swapCatsAtPositions("+p1+","+p2+") out of range ("+cats+" cats)")!=-1;
    ...
    public boolean swapCats(int c1, int c2) {
        if (c1<0 || c2<0 || c1>=cats || c2>=cats) return
            Common.runtimeWarning("SCatSequence on "+v.getName()+": swapCats("+c1+","+c2+") out of range ("+cats+" cats)")!=-1;
        if (seqToCat==null) createFields();
        int p1=catToSeq[c1];
        int p2=catToSeq[c2];
        seqToCat[p1]=c2; seqToCat[p2]=c1;
    ...
    public boolean moveCatAtPosTo(int p1, int p2) {
        ...
        if (p1<0 || p2<0 || p1>=cats || p2>=cats) return
            Common.runtimeWarning("SCatSequence on "+v.getName()+": moveCatAtPosTo("+p1+","+p2+") out of range ("+cats+" cats)")!=-1;
        if (seqToCat==null) createFields();
        int c1=seqToCat[p1];
