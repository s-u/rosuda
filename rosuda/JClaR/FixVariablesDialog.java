...
public final class FixVariablesDialog extends TableDialog implements SelectionModIF {
     ...
     */
    static final long serialVersionUID = 200602271220L;
    ...
    private final FixVariablesSliderPanel fvsp;    
    /**
            ...
            public final void stateChanged(final javax.swing.event.ChangeEvent evt) {
                if(updateDevVal){
                    if (fvsp.whichSlider(evt.getSource())==1){
                        hardChange=true;
                        final double newValue = 8*sds[lastSelectedRow]*fvsp.getValue()-(double)50/100+medians[lastSelectedRow];
                        tm.setValue(newValue,lastSelectedRow);
                    } else {
                        final double newDeviation = 4*sds[lastSelectedRow]*fvsp.getDeviation()/100;
    ...
    private void setData(final Data data, final int varPos){
        ...
        for (int i=0; i<numVars;i++){
            ...
            try{
                values[i] = RserveConnection.evalD("as.double(median(" + data.getRname() + "[," + j + "]))");
                deviations[i] = RserveConnection.evalD("as.double(sd(" + data.getRname() + "[," + j + "])/3)");
    ...
    private void setSliderDeviation(final double deviation){
        updateDevVal=false;
        fvsp.setDeviation((int)(deviation/sds[lastSelectedRow]*100/4));
        updateDevVal=true;
    }
    
    private void setSliderValue(final double value){
        updateDevVal=false;
        fvsp.setValue((int)((value-medians[lastSelectedRow])/sds[lastSelectedRow]*100/8)+50);
        updateDevVal=true;
    }
    ...
    private boolean evaluate(){
        ...
        if (j!=2) {
            ...
            subsetExpression="";
            boolean comma = false;
            for(final ListIterator e = fixedVariablesVector.listIterator(); e.hasNext();){
                if(comma) {
                    fixedVariables += ",";
                    subsetExpression += " & ";
                } else  {
                ...
                final int i=((Integer)e.next()).intValue();
