	...
	public String[] getParamCalls() {
		...
		if(getValue()!=null && !getValue().equals(getDefaultValue())){
			...
			if(getDefaultValue()==null || (getValue()!=null && !getDefaultValue().toString().equals(getValue().toString()))){
				if(format.equals(FORMAT_VECTOR)){
					calls = new String[] {getName() +" = " + data +"$"+variable};
				}else if(format.equals(FORMAT_DATA)){
					calls = new String[] {getName() +" = " + data +"[,'"+variable+"']"};
				}else if(format.equals(FORMAT_VARIABLE)){
					calls = new String[] {getName() +" = " +variable ,dataParamName + " = "+ data};
				}else if(format.equals(FORMAT_VARIABLE_CHARACTER)){
					calls = new String[] {getName() +" = '" +variable+"'" , dataParamName + " = "+ data};
				}
			}else
				val ="";
