...
public class PC extends DragBox implements ActionListener {
  ...
  protected double[] Mins, MINS;
  protected double[] Maxs, MAXS;
  ...
  public void processMouseEvent(MouseEvent e) {
    ...
    if (e.getID() == MouseEvent.MOUSE_PRESSED ||
      ...
      if (e.isPopupTrigger() && e.getModifiers()!=24 ) {
        ...
        if( minXDist < slotWidth/4 ) {
          ...
          if(  data.phoneNumber(vars[popXId]) ) {
            colName = new JMenuItem(data.getName(vars[popXId])+'\n'+"Number: "+ Util.toPhoneNumber(dataCopy[popXId][popYId]));
          }
          else if( data.categorical(vars[popXId]) )
            if( paintMode.equals("Poly") ) {
              colName = new JMenuItem(data.getName(vars[popXId]));
              colVal  = new JMenuItem("   Level: "+data.getLevelName(vars[popXId], dataCopy[popXId][popYId]));
            }
              else
                for( int i = 0;i < rects.size(); i++) {
                  ...
                  if ( r.contains( e.getX(), e.getY()+sb.getValue() )) {
                    ...
                    colName = new JMenuItem(data.getName(vars[popXId]));
                    colVal  = new JMenuItem("   Value: "+dataCopy[popXId][popYId]);
                  }
                  if( colName != null ) {
                    name.add(colName);
                    name.add(colVal);
 ...
 //System.out.println("Moving Stop");
          ...
          int save = vars[movingID];  
          if(movingID < insertBefore) {
            for( int j=movingID; j<insertBefore; j++ )
              vars[j] = vars[j+1];
            vars[insertBefore-1] = save;
          }
          else {
            for( int j=movingID; j>insertBefore; j-- )
              vars[j] = vars[j-1];
            vars[insertBefore] = save;
          }
...
//System.out.println("lastX: "+lastX);
              ...
              if( Math.abs(e.getX() - lastX) > 4 ) {
                int testX = lastX + slotWidth/2 * ( e.getX() - lastX ) / Math.abs( e.getX() - lastX );
                for( int j=0; j<k; j++ ) {
...
//System.out.println("popXId: "+popXId+"e.getX: "+e.getX());
                ...
                setCoordinates(0, Mins[popXId], 1, Maxs[popXId], 0);
                ...
                Mins[popXId] = MINS[popXId];
                Maxs[popXId] = MAXS[popXId];
              }
 //System.out.println("Id: "+popXId);
  ...
  /* public void processKeyEvent(KeyEvent e) {
    ...
    System.out.println("Got Key Event in PC");
  ...
  public void paint(Graphics g2d) {
    ...
    if( printing ) {
      g.setClip(0,0,printerPage.width, printerPage.height);
System.out.println("we are printing ............:"+printerPage+"..."+g.getClipBounds());
//      size = pj.getPageDimension();
    ...
    if( bg == null || printing || g instanceof PSGr ) {
      ...
      for( int j=0; j<k; j++ ) {
        int x = border+addBorder+(j*slotWidth);
    ...
    for( int i=0; i<Selections.size(); i++) {
      ...
      for( int j=0; j<k; j++)
        if( (  paintMode.equals("XbyY") && j == ((floatRect)S.o).var ) ||
            ( !paintMode.equals("XbyY") && vars[j] == ((floatRect)S.o).var ) )
          plotID = j;
      ...
      S.r.width  = (int)(((floatRect)S.o).w * slotWidth);
...
//System.out.println(" **** select col: "+selectCol);
    ...
    void getData() {
      ...
      for( int j=0; j<k; j++ ) {
        Mins[j] = data.getMin(vars[j]);
        MINS[j] = Mins[j];
        Maxs[j] = data.getMax(vars[j]);
        MAXS[j] = Maxs[j];
  ...
  void create(int width, int height) {
    ...
    for( int j=0; j<k; j++ ) {
      if( !paintMode.equals("XbyY") )
        names.addElement(new MyText(data.getName(vars[j]), 1, 1));
      else {
    ...
    for( int i=0; i<data.n; i++ ) {
      poly[i] = new Polygon();
      for( int j=0; j<k; j++ ) {	
        int x = border+addBorder+slotWidth*j;
        int y = (int)(-border + height - (height-2*border) * ((dataCopy[j][i] - Mins[j])/(Maxs[j]-Mins[j])));
        poly[i].addPoint(x, y);
      }
    }
    if( paintMode.equals("Box") || paintMode.equals("Both") || paintMode.equals("XbyY") ) {
...
//        System.out.println("Name: "+lNames[j]);
        ...
        if( !data.categorical(vars[j]) ) {
          if( paintMode.equals("XbyY") )
            data.setFilter( yVar, lNames[j]);
          bPlots.addElement(new boxPlot( j, vars[j] , x + addBorder, Math.min(slotWidth/2, 40), border, height-border));
          data.resetFilter();
        }
        else {
          int[] dummy = new int[1];
    ...
    */
    setDragBoxConstraints(0, 0, width, height, slotWidth-2, height);
  }
