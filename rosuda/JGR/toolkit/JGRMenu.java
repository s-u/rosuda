    ...
    public static JMenuBar getMenu(JFrame f, ActionListener al, String[] menuDef) {
        ...
        while (menuDef[i]!="0") {
            ...
            if (menuDef[i]=="+") {
                ...
                if (m==null) {
                	mb.add(m=new JMenu(menuDef[i].substring(menuDef[i].charAt(0)=='$'?2:0)));
                	if (menuDef[i].charAt(0)=='$') m.setMnemonic(menuDef[i].charAt(1));
                }
                i++;
            };
            ...
            if (!isNext) {
                ...
                if (mi==null) {
                    if (menuDef[i].charAt(0)=='#') {
                        m.add(mi=new JMenu(menuDef[i].substring(menuDef[i].charAt(1)=='$'?3:1)));
                        if (menuDef[i].charAt(1)=='$') mi.setMnemonic(menuDef[i].charAt(2));
                    }
                    else if (menuDef[i].charAt(0)=='@' || menuDef[i].charAt(0)=='!') {
                        mi=new JMenuItem(menuDef[i].substring(menuDef[i].charAt(2)=='$'?4:2));
                        mi.setAccelerator(javax.swing.KeyStroke.getKeyStroke(menuDef[i].charAt(1), Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()+((menuDef[i].charAt(0)=='!')?1:0),false));
                        if (menuDef[i].charAt(2)=='$') mi.setMnemonic(menuDef[i].charAt(3));
                        mi.setActionCommand(rac);
                        m.add(mi);
                    } else {
                    	mi=new JMenuItem(menuDef[i].substring(menuDef[i].charAt(0)=='$'?2:0));
                        if (menuDef[i].charAt(0)=='$') mi.setMnemonic(menuDef[i].charAt(1));
                        mi.setActionCommand(rac);
                        m.add(mi);
                    }
                    mi.addActionListener(al);
