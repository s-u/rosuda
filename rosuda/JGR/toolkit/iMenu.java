	...
	public static JMenuBar getMenu(JFrame f, ActionListener al, String[] menuDef) {
		...
		while (menuDef[i] != "0") {
			...
			if (menuDef[i] == "~File.Graph") {
				...
				mi.setAccelerator(javax.swing.KeyStroke.getKeyStroke('P',
				...
				mi.setAccelerator(javax.swing.KeyStroke
						.getKeyStroke('P', Toolkit.getDefaultToolkit()
				...
				if (Common.supportsBREAK) {
					...
					mi.setAccelerator(javax.swing.KeyStroke.getKeyStroke('B',
							Toolkit.getDefaultToolkit()
									.getMenuShortcutKeyMask() + 1, false));
				...
				if (mi == null) {
					if (menuDef[i].charAt(0) == '#')
						...
						if (menuDef[i].charAt(0) == '@'
							...
							m.add(mi = new JMenuItem(menuDef[i]))
									.setActionCommand(rac);
						mi.addActionListener(al);
					}
