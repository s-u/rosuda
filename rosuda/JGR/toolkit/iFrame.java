...
/**
 * 
 * iFrame - every window in JGR is based on iFrame, iFrame takes care about the
 * window menu and the same look and feel for every frame.
 * 
		...
		this.addComponentListener(new ComponentListener() {
			public void componentResized(ComponentEvent e) {
				if (!Common.isMac()) {
					...
					if (minimumSize != null && getHeight() < minimumSize.height) {
						resize = true;
						h = minimumSize.height;
					}
					if (minimumSize != null && getWidth() < minimumSize.width) {
						resize = true;
						w = minimumSize.width;
					}
					if (resize)
