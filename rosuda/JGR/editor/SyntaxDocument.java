...
public class SyntaxDocument extends DefaultStyledDocument {
     ...
     */
    private synchronized void processChangedLines(int offset, int length) throws
        BadLocationException {
    ...
    private synchronized void applyHighlighting(String content, int line) {
        ...
        if ( (index > -1) && (index < endOffset)) {
            doc.setCharacterAttributes(index, endOffset - index + 1,
            		EditorPreferences.COMMENT, false);
            endOffset = index - 1;
        }
        // check for tokens*/
    ...
    private synchronized int getOtherToken(String content, int startOffset, int endOffset) {
        ...
        if (isKeyword(token))
            doc.setCharacterAttributes(startOffset, endOfToken - startOffset,
                                       EditorPreferences.KEYWORD, false);
        if (isObject(token)) 
            doc.setCharacterAttributes(startOffset, endOfToken - startOffset,
                                       EditorPreferences.OBJECT, false);
        else if (isNumber(token))
            doc.setCharacterAttributes(startOffset, endOfToken - startOffset,
                                       EditorPreferences.NUMBER, false);
        return endOfToken + 1;
    }
